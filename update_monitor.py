...
def detect_co(lines, firms):
    """
    Rozpoznaje firmę po numerze konta w nagłówku CSV (40 linii, bez spacji).
    Strategia: najpierw pełny numer konta, potem suffix — od najdłuższego.
    Unikamy fałszywych trafień dla krótkich suffixów jak '5032'.
    """
    header    = "\n".join(lines[:40])
    header_ns = re.sub(r"\s", "", header)

    # Sortuj malejąco po długości suffiksu — dłuższy = bardziej unikalny
    sorted_firms = sorted(firms, key=lambda f: len(re.sub(r"\s","",f.get("konto_suffix",""))), reverse=True)

    # Próba 1: pełny numer konta (26 cyfr, bez spacji)
    for f in sorted_firms:
        full_ns = re.sub(r"\s", "", f.get("konto", ""))
        if full_ns and len(full_ns) >= 20 and full_ns in header_ns:
            return f["co"]

    # Próba 2: suffix ≥ 6 cyfr
    for f in sorted_firms:
        suf = re.sub(r"\s", "", f.get("konto_suffix", ""))
        if len(suf) >= 6 and suf in header_ns:
            return f["co"]

    # Próba 3: krótki suffix (fallback)
    for f in sorted_firms:
        suf = re.sub(r"\s", "", f.get("konto_suffix", ""))
