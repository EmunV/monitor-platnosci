...
def parse_inv_in(item, co):
    ...
    selling = (item.get("SellingParty") or item.get("Seller") or
               item.get("Contractor") or item.get("Vendor") or {})
    partner = (selling.get("Name") or selling.get("CompanyName") or
               item.get("SellingPartyName") or item.get("ContractorName") or
               item.get("SupplierName") or item.get("VendorName") or
               item.get("CustomerName") or item.get("IssuerName") or
               # Betterfly v1.4 vatpurchasebooks używa innych pól
               item.get("SellerName") or item.get("DocumentIssuer") or
               item.get("CompanyName") or "").strip()
    nip_partner = (selling.get("TaxNumber") or selling.get("NIP") or
                   selling.get("Nip") or item.get("SupplierNIP") or
                   item.get("SellerNIP") or item.get("TaxNumber") or "").strip()
    # Loguj jeśli brak partnera — pomocne przy debugowaniu
    if not partner:
        available_keys = list(item.keys())[:15]
        print(f"    [DEBUG] parse_inv_in brak partnera, dostepne pola: {available_keys}")
...
def match_nr_safe(nr, text):
    """
    Dopasowanie numeru faktury w tekście.
    Numer musi być otoczony separatorami — nie może być częścią większej liczby.
    Np. FV/4/06/2026 NIE pasuje do opisu '20260617-4'.
    """
    nr_n = norm(nr)
    if len(nr_n) < MIN_NR_LEN:
        return False
    text_n = norm(text)
    if nr_n not in text_n:
        return False
    idx    = text_n.find(nr_n)
    before = text_n[idx - 1] if idx > 0 else ""
    after  = text_n[idx + len(nr_n)] if idx + len(nr_n) < len(text_n) else ""
    if before.isalnum() or after.isalnum():
        return False
    return True

def date_ok_for_match(inv_date, tx_date, max_before=30, max_after=180):
    """Transakcja powinna być max 30 dni przed fakturą i max 180 dni po."""
    if not inv_date or not tx_date:
        return True
    try:
        id_ = datetime.strptime(inv_date[:10], "%Y-%m-%d")
        td_ = datetime.strptime(tx_date[:10],  "%Y-%m-%d")
        return -max_before <= (td_ - id_).days <= max_after
    except Exception:
        return True
...
def run_matching(fv, tx, firms_by_co=None):
    ...
    for inv in fv:
        ...
        if nr:
            for t in tx_by_co[co]:
                if t.get("ref") or not dir_ok(t):
                    continue
                if not date_ok_for_match(inv_date, t["dt"]):
                    continue
                if match_nr_safe(nr, t["dc"]) or                    (t.get("inv_nr") and match_nr_safe(nr, t["inv_nr"])):
        ...
        # Weryfikacja daty: transakcja max 30 dni przed fakturą, max 180 dni po
        candidates = [t for t in amt_idx[(co, brutto)]
                      if not t.get("ref") and dir_ok(t)
                      and date_ok_for_match(inv_date, t["dt"])]

        best, best_sc = None, 0.0
        for t in candidates:
            sc = partner_score(partner, t["dc"])
            # Bonus za NIP kontrahenta w opisie przelewu
            if nip_p and nip_in_desc(nip_p, t["dc"]):
                sc = min(1.0, sc + 0.4)
            # Bonus za datę bliską terminowi płatności
            if inv.get("due") and date_ok_for_match(inv.get("due",""), t["dt"], 7, 30):
                sc = min(1.0, sc + 0.15)
            if sc > best_sc:
                best_sc = sc; best = t
