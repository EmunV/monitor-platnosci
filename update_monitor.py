...
def run_matching(fv, tx):
    ...
    for inv in fv:
        ...
        # L3: amount within 1% + partner match (check close amounts only)
        for t in tx_by_co.get(co,[]):
            if t.get("ref"): continue
            ta = float(t["a"])
...
def build_html(fv,tx,updated):
    ...
    fv_b64 = _b64.b64encode(fv_json.encode()).decode()
    tx_b64 = _b64.b64encode(tx_json.encode()).decode()
    html = tpl
...
def main():
    ...
    for firm in firms:
        ...
        try:
            ...
            for ep in endpoints_in:
                # vatpurchasebooks is slower - use smaller page size
                ep_params = dict(params)
                if 'vatpurchasebooks' in ep:
                    ep_params['pageSize'] = 200
                items = api_get_all(token, ep, ep_params, ep)
                for item in items:
            ...
            for item in raw_out:
                ...
                if not r["nr"]:  # fallback nr for vatpurchasebooks
    ...
    print(f"\nDopasowywanie faktur do transakcji ({len(all_fv)} FV x {len(all_tx)} TX)...")
    import time as _time
    _t0 = _time.time()
    all_fv, all_tx, matched = run_matching(all_fv, all_tx)
    print(f"  Czas dopasowania: {round(_time.time()-_t0)}s")
    ...
    # Step 2: only from directly matched transactions - no propagation by partner name
    # (too risky for bailiffs/courts who have many different case numbers)
    case_from_partner = 0
    with_case = sum(1 for f in all_fv if f.get("fv_case"))
    print(f"  Numery spraw: {with_case} FV (tylko z dopasowanych transakcji)")
