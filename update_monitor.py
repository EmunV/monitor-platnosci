...
def parse_inv_out(item, co):
    ...
    return {"co": co, "t": "out", "nr": nr.strip(), "data": issued,
            ...
            "partial": False, "fv_case": "", "match_level": 0}
...
def parse_inv_in(item, co):
    ...
    return {"co": co, "t": "in", "nr": nr.strip(), "data": issued,
            "partner": partner.strip(), "nabywca": partner.strip(),
...
def extract_case(desc):
    ...
    for pat,lbl in CASE_PATTERNS:
        for m in re.finditer(pat,desc,re.IGNORECASE):
            k=f"{lbl} {re.sub(r'(\\d)\\s+(\\d)',r'\\1/\\2',m.group(1).strip())}"
...
def parse_bank_csv(filepath, co, existing_keys):
    ...
    for line in lines[hdr+1:]:
        ...
        txs.append({"co":co,"id":len(txs)+1,"dt":dt,"dc":dc[:120],
                    "a":amt,"ref":None,"case":extract_case(dc)})
    return txs
...
def run_matching(fv, tx):
    ...
    for inv in fv:
        ...
        # L1: invoice number in TX description
    ...
    for inv in all_fv:
        if inv.get("mx"):
            ...
            if tx and tx.get("case"):
                inv["fv_case"] = tx["case"]
        if not inv.get("fv_case") and inv.get("partner"):
            tokens = norm_tokens(inv["partner"])
            co_txs = [t for t in all_tx if t["co"]==inv["co"] and t.get("case")]
            for tx in co_txs:
                td = tx["dc"].upper()
                for a,b in [("Ą","A"),("Ć","C"),("Ę","E"),("Ł","L"),("Ń","N"),("Ó","O"),("Ś","S"),("Ź","Z"),("Ż","Z")]:
                    td = td.replace(a,b)
                if sum(1 for tk in tokens if tk in td) >= min(2, len(tokens)):
                    inv["fv_case"] = tx["case"]
                    case_from_partner += 1
                    break
    with_case = sum(1 for f in all_fv if f.get("fv_case"))
    print(f"  Numery spraw: {with_case} FV (w tym {case_from_partner} przez nazwe nabywcy)")
